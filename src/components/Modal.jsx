            ...
            <div className='bg-white p-2 relative rounded-md w-72 md:w-96 h-32'>
                ...
                <div className='p-6 flex  items-center justify-center space-x-3'>
                    <div className=' border-dashed border w-64 overflow-hidden border-gray-400 rounded-md px-3 py-1 text-sm '>
                        <p className='text-gray-500'>Link:<a ref={copyLink} target='_blank' rel="noreferrer" href={`https://pdeepak936.github.io/flashcard-generator/card/${groupId}/${cardId}`}
                        className='ml-1 text-blue-500'>{`https://pdeepak936.github.io/flashcard-generator/card/${groupId}/${cardId}`}</a></p>
                    </div>
                    <FaRegCopy
                        onClick={handleCopy}
